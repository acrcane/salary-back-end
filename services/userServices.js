...
// add ifCompanyExists & logic
export const ifCompanyExists = async companyName => {
  const company = await User.findOne({ company: companyName });
  return company;
};
...
export const createUser = async userData => {
  ...
  if(!await ifCompanyExists(userData.company)){
    user.role = 'manager'
  }
  await user.save();
